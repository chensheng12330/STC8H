...
INT8U  OSQPendAbort_FROM_ISR (OS_EVENT  *pevent,
                     INT8U      opt,
                     INT8U     *perr)FunctionProperties
{
    INT8U      nbr_tasks;
    ...
    if (pevent->OSEventGrp != 0u) {                        /* See if any task waiting on queue?        */
        ...
        OS_EXIT_CRITICAL();
        OS_Sched_FROM_ISR();                               /* Find HPT ready to run                    */
        *perr = OS_ERR_PEND_ABORT;
        return (nbr_tasks);
    }
    OS_EXIT_CRITICAL();
...
INT8U  OSQPost_FROM_ISR (OS_EVENT  *pevent,
                void      *pmsg)FunctionProperties
{
    OS_Q      *pq;
#if OS_CRITICAL_METHOD == 3u                           /* Allocate storage for CPU status register     */
    ...
    if (pevent->OSEventGrp != 0u) {                    /* See if any task pending on queue             */
        ...
        (void)OS_EventTaskRdy(pevent, pmsg, OS_STAT_Q, OS_STAT_PEND_OK);
        OS_EXIT_CRITICAL();
        OS_Sched_FROM_ISR();                           /* Find highest priority task ready to run      */
        OS_TRACE_Q_POST_EXIT(OS_ERR_NONE);
        return (OS_ERR_NONE);
    }
    pq = (OS_Q *)pevent->OSEventPtr;                   /* Point to queue control block                 */
...
INT8U  OSQPostFront_FROM_ISR (OS_EVENT  *pevent,
                     void      *pmsg)FunctionProperties
{
    OS_Q      *pq;
#if OS_CRITICAL_METHOD == 3u                          /* Allocate storage for CPU status register      */
...
INT8U  OSQPostOpt_FROM_ISR (OS_EVENT  *pevent,
                   void      *pmsg,
                   INT8U      opt)FunctionProperties
{
    OS_Q      *pq;
