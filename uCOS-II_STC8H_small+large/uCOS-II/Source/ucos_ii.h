...
#if OS_TMR_EN > 0u
typedef  void (*OS_TMR_CALLBACK)(void *ptmr, void *parg)FunctionProperties;
...
typedef  void                      (*OS_TLS_DESTRUCT_PTR)(OS_TCB    *ptcb,
                                                          OS_TLS_ID  id,
                                                          OS_TLS     value)FunctionProperties;
#endif
#endif
...
OS_EXT  INT32U xdata      OSCtxSwCtr;               /* Counter of number of context switches           */

#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0u)
OS_EXT  OS_EVENT * xdata  OSEventFreeList;          /* Pointer to list of free EVENT control blocks    */
OS_EXT  OS_EVENT xdata    OSEventTbl[OS_MAX_EVENTS];/* Table of EVENT control blocks                   */
#endif

#if (OS_FLAG_EN > 0u) && (OS_MAX_FLAGS > 0u)
OS_EXT  OS_FLAG_GRP xdata OSFlagTbl[OS_MAX_FLAGS];  /* Table containing event flag groups              */
OS_EXT  OS_FLAG_GRP *xdata OSFlagFreeList;           /* Pointer to free list of event flag groups       */
#endif

#if OS_TASK_STAT_EN > 0u
OS_EXT  INT8U    xdata    OSCPUUsage;               /* Percentage of CPU used                          */
OS_EXT  INT32U   xdata    OSIdleCtrMax;             /* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  INT32U   xdata    OSIdleCtrRun;             /* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN  xdata    OSStatRdy;                /* Flag indicating that the statistic task is rdy  */
...
OS_EXT  INT8U   xdata     ID_STK_Size;                /* idata公共运行栈大小                            */
OS_EXT  INT8U   xdata     ID_STK_Free;                /* idata公共运行栈未使用                          */
OS_EXT  INT8U   xdata     ID_STK_Used;                /* idata公共运行栈已使用                          */
OS_EXT  INT8U   xdata     ID_CH_Start;                /* 从此处开始检查                                 */
...
OS_EXT  INT8U   xdata     OSTaskCtr;                       /* Number of tasks created                  */

OS_EXT  volatile  INT32U xdata OSIdleCtr;                  /* Idle counter                   */

#ifdef OS_SAFETY_CRITICAL_IEC61508
OS_EXT  BOOLEAN  xdata    OSSafetyCriticalStartFlag;
#endif
...
OS_EXT  OS_TCB * xdata    OSTCBFreeList;                   /* Pointer to list of free TCBs             */
...
OS_EXT  OS_TCB * xdata    OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];    /* Table of pointers to created TCBs   */
OS_EXT  OS_TCB   xdata    OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS];   /* Table of TCBs                  */

#if OS_TICK_STEP_EN > 0u
OS_EXT  INT8U    xdata    OSTickStepState;          /* Indicates the state of the tick step feature    */
#endif

#if (OS_MEM_EN > 0u) && (OS_MAX_MEM_PART > 0u)
OS_EXT  OS_MEM * xdata    OSMemFreeList;            /* Pointer to free list of memory partitions       */
OS_EXT  OS_MEM   xdata    OSMemTbl[OS_MAX_MEM_PART];/* Storage for memory partition manager            */
#endif

#if (OS_Q_EN > 0u) && (OS_MAX_QS > 0u)
OS_EXT  OS_Q *   xdata    OSQFreeList;              /* Pointer to list of free QUEUE control blocks    */
OS_EXT  OS_Q     xdata    OSQTbl[OS_MAX_QS];        /* Table of QUEUE control blocks                   */
#endif

#if OS_TASK_REG_TBL_SIZE > 0u
OS_EXT  INT8U    xdata    OSTaskRegNextAvailID;     /* Next available Task register ID                 */
#endif

#if OS_TIME_GET_SET_EN > 0u
OS_EXT  volatile  INT32U xdata OSTime;              /* Current value of system time (in ticks)         */
#endif

#if OS_TMR_EN > 0u
OS_EXT  INT16U  xdata     OSTmrFree;                /* Number of free entries in the timer pool        */
OS_EXT  INT16U  xdata     OSTmrUsed;                /* Number of timers used                           */
OS_EXT  INT32U  xdata     OSTmrTime;                /* Current timer time                              */

OS_EXT  OS_EVENT * xdata  OSTmrSem;                 /* Sem. used to gain exclusive access to timers    */
OS_EXT  OS_EVENT * xdata  OSTmrSemSignal;           /* Sem. used to signal the update of timers        */

OS_EXT  OS_TMR   xdata    OSTmrTbl[OS_TMR_CFG_MAX]; /* Table containing pool of timers                 */
OS_EXT  OS_TMR * xdata    OSTmrFreeList;            /* Pointer to free list of timers                  */
OS_EXT  OS_STK   xdata    OSTmrTaskStk[OS_TASK_TMR_STK_SIZE];

OS_EXT  OS_TMR_WHEEL xdata OSTmrWheelTbl[OS_TMR_CFG_WHEEL_SIZE];
#endif
...
OS_TLS_ID     OS_TLS_GetID            (INT8U               *perr)FunctionProperties;

OS_TLS        OS_TLS_GetValue         (OS_TCB              *ptcb,
                                       OS_TLS_ID            id,
                                       INT8U               *perr)FunctionProperties;

void          OS_TLS_Init             (INT8U               *perr)FunctionProperties;
...
void          OS_TLS_TaskCreate       (OS_TCB              *ptcb)FunctionProperties;

void          OS_TLS_TaskDel          (OS_TCB              *ptcb)FunctionProperties;

void          OS_TLS_TaskSw           (void)FunctionProperties;
...
OS_FLAGS      OSFlagPendGetFlagsRdy   (void)FunctionProperties;
...
void         *OSMboxAccept            (OS_EVENT        *pevent)FunctionProperties;
#endif

OS_EVENT     *OSMboxCreate            (void            *pmsg)FunctionProperties;
...
INT8U         OSMboxPost              (OS_EVENT        *pevent,
                                       void            *pmsg)FunctionProperties;
#endif

#if OS_MBOX_POST_OPT_EN > 0u
INT8U         OSMboxPostOpt           (OS_EVENT        *pevent,
                                       void            *pmsg,
                                       INT8U            opt)FunctionProperties;
#endif

#if OS_MBOX_QUERY_EN > 0u
INT8U         OSMboxQuery             (OS_EVENT        *pevent,
                                       OS_MBOX_DATA    *p_mbox_data)FunctionProperties;
#endif
#endif
...
INT8U         OSMemPut                (OS_MEM          *pmem,
                                       void            *pblk)FunctionProperties;

#if OS_MEM_QUERY_EN > 0u
INT8U         OSMemQuery              (OS_MEM          *pmem,
                                       OS_MEM_DATA     *p_mem_data)FunctionProperties;
#endif
...
INT8U         OSMutexPost             (OS_EVENT        *pevent)FunctionProperties;

#if OS_MUTEX_QUERY_EN > 0u
INT8U         OSMutexQuery            (OS_EVENT        *pevent,
                                       OS_MUTEX_DATA   *p_mutex_data)FunctionProperties;
#endif
...
OS_EVENT     *OSQCreate               (void           **start,
                                       INT16U           size)FunctionProperties;
...
#if OS_Q_FLUSH_EN > 0u
INT8U         OSQFlush                (OS_EVENT        *pevent)FunctionProperties;
#endif
...
INT8U         OSQQuery                (OS_EVENT        *pevent,
                                       OS_Q_DATA       *p_q_data)FunctionProperties;
#endif
...
INT16U        OSSemAccept             (OS_EVENT        *pevent)FunctionProperties;
#endif

OS_EVENT     *OSSemCreate             (INT16U           cnt)FunctionProperties;
...
INT8U         OSSemPost               (OS_EVENT        *pevent)FunctionProperties;

#if OS_SEM_QUERY_EN > 0u
INT8U         OSSemQuery              (OS_EVENT        *pevent,
                                       OS_SEM_DATA     *p_sem_data)FunctionProperties;
#endif
...
INT8U         OSTaskChangePrio        (INT8U            oldprio,
                                       INT8U            newprio)FunctionProperties;
#endif
...
INT8U         OSTaskCreateExt         (void           (*task)(void *p_arg),
                                       ...
                                       INT32U           stk_size,
                                       void            *pext,
                                       INT16U           opt)FunctionProperties;
#endif

#if OS_TASK_DEL_EN > 0u
INT8U         OSTaskDel               (INT8U            prio)FunctionProperties;
INT8U         OSTaskDelReq            (INT8U            prio)FunctionProperties;
#endif
...
INT8U         OSTaskResume            (INT8U            prio)FunctionProperties;
INT8U         OSTaskSuspend           (INT8U            prio)FunctionProperties;
#endif

#if (OS_TASK_STAT_STK_CHK_EN > 0u) && (OS_TASK_CREATE_EXT_EN > 0u)
INT8U         OSTaskStkChk            (INT8U            prio,
                                       OS_STK_DATA     *p_stk_data)FunctionProperties;
#endif

#if OS_TASK_QUERY_EN > 0u
INT8U         OSTaskQuery             (INT8U            prio,
                                       OS_TCB          *p_task_data)FunctionProperties;
#endif
...
INT8U         OSTaskRegGetID          (INT8U           *perr)FunctionProperties;
...
void          OSTimeDly               (INT32U           ticks)FunctionProperties;

#if OS_TIME_DLY_HMSM_EN > 0u
INT8U         OSTimeDlyHMSM           (INT8U            hours,
                                       INT8U            minutes,
                                       INT8U            seconds,
                                       INT16U           ms)FunctionProperties;
#endif

#if OS_TIME_DLY_RESUME_EN > 0u
INT8U         OSTimeDlyResume         (INT8U            prio)FunctionProperties;
#endif

#if OS_TIME_GET_SET_EN > 0u
INT32U        OSTimeGet               (void)FunctionProperties;
void          OSTimeSet               (INT32U           ticks)FunctionProperties;
#endif

void          OSTimeTick_FROM_ISR     (void)small;
...
INT8U        OSTmrSignal              (void)FunctionProperties;
#endif
...
void          OSInit                  (void)FunctionProperties;

void          OSIntEnter              (void)FunctionProperties;
void          OSIntExit               (void)FunctionProperties;

#ifdef OS_SAFETY_CRITICAL_IEC61508
void          OSSafetyCriticalStart   (void)FunctionProperties;
#endif

#if OS_SCHED_LOCK_EN > 0u
void          OSSchedLock             (void)FunctionProperties;
void          OSSchedUnlock           (void)FunctionProperties;
#endif

void          OSStart                 (void)FunctionProperties;

void          OSStatInit              (void)FunctionProperties;

INT16U        OSVersion               (void)FunctionProperties;
...
#if OS_TASK_DEL_EN > 0u
void          OS_Dummy                (void)FunctionProperties;
#endif
...
INT8U         OS_EventTaskRdy         (OS_EVENT        *pevent,
                                       void            *pmsg,
                                       INT8U            msk,
                                       INT8U            pend_stat)FunctionProperties;

void          OS_EventTaskWait        (OS_EVENT        *pevent)FunctionProperties;

void          OS_EventTaskRemove      (OS_TCB          *ptcb,
                                       OS_EVENT        *pevent)FunctionProperties;

#if (OS_EVENT_MULTI_EN > 0u)
void          OS_EventTaskWaitMulti   (OS_EVENT       **pevents_wait)FunctionProperties;

void          OS_EventTaskRemoveMulti (OS_TCB          *ptcb,
                                       OS_EVENT       **pevents_multi)FunctionProperties;
#endif

void          OS_EventWaitListInit    (OS_EVENT        *pevent)FunctionProperties;
#endif

#if (OS_FLAG_EN > 0u) && (OS_MAX_FLAGS > 0u)
void          OS_FlagInit             (void)FunctionProperties;
void          OS_FlagUnlink           (OS_FLAG_NODE    *pnode)FunctionProperties;
#endif
...
#if (OS_MEM_EN > 0u) && (OS_MAX_MEM_PART > 0u)
void          OS_MemInit              (void)FunctionProperties;
#endif

#if OS_Q_EN > 0u
void          OS_QInit                (void)FunctionProperties;
#endif

void          OS_Sched                (void)FunctionProperties;
void          OS_Sched_FROM_ISR       (void)FunctionProperties;

#if (OS_EVENT_NAME_EN > 0u) || (OS_FLAG_NAME_EN > 0u) || (OS_MEM_NAME_EN > 0u) || (OS_TASK_NAME_EN > 0u)
INT8U         OS_StrLen               (INT8U           *psrc)FunctionProperties;
#endif

void          OS_TaskIdle             (void            *p_arg)large;

void          OS_TaskReturn           (void)FunctionProperties;

#if OS_TASK_STAT_EN > 0u
void          OS_TaskStat             (void            *p_arg)large;
#endif
...
#if OS_TMR_EN > 0u
void          OSTmr_Init              (void)FunctionProperties;
#endif
...
#if OS_DEBUG_EN > 0u
void          OSDebugInit             (void)FunctionProperties;
#endif
...
void          App_TaskCreateHook      (OS_TCB          *ptcb)FunctionProperties;
void          App_TaskDelHook         (OS_TCB          *ptcb)FunctionProperties;
void          App_TaskIdleHook        (void)FunctionProperties;

void          App_TaskReturnHook      (OS_TCB          *ptcb)FunctionProperties;

void          App_TaskStatHook        (void)FunctionProperties;

#if OS_TASK_SW_HOOK_EN > 0u
void          App_TaskSwHook          (void)FunctionProperties;
#endif

void          App_TCBInitHook         (OS_TCB          *ptcb)FunctionProperties;

#if OS_TIME_TICK_HOOK_EN > 0u
void          App_TimeTickHook        (void)FunctionProperties;
#endif
#endif
...
* 并且不会引起任务切换，就是通用服务，即可以任务级调用，也可中断级调用. 否则不可中断调用.
*/


/*
* 针对所有需要中断调用并且会引起任务切换的OS服务，移植版本提供了中断调用版本，以下是新增中断服务的函数
*/
