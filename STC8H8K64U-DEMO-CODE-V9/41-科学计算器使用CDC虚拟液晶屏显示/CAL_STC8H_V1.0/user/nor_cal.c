...
a:          Cursor_Blink((u8)(addr*8),0,50); 
        ...
        case 2://计算结果
            ...
            {
                sprintf(output_buf,"%g",(double)re_result);
                g_ans = re_result;
...
{
    static xdata u8 addr=0;
    u8 xdata result=0;
    static u8 xdata shift_flag=0;
    double xdata re_result=0,im_result=0;       //计算结果
	char xdata input_buf[IBlength];     //输入缓存
	char xdata output_buf[16];    //输出缓存   
...
a:          Cursor_Blink((u8)(addr*8),0,50); 
        ...
        case 2://计算结果
            ...
            {
				if(im_result <0.0)
					sprintf(output_buf,"%g%gi",(double)re_result,(double)im_result);
				else
					sprintf(output_buf,"%g+%gi",(double)re_result,(double)im_result);
