...
double xdata a_shuzhi = 0;
double xdata b_shuzhi = 0;
double xdata c_shuzhi = 0;
double xdata d_shuzhi = 0;
double xdata e_shuzhi = 0;
double xdata x0_shuzhi= 0;
double xdata x_shuzhi = 0;	//x是求得的根 

u8 xdata solvestr[12];
u8 xdata solvestrlength=0;
u8 xdata solvestrnum=0;
...
{
	double xdata x1;
	uchar xdata wrong[6]  = "wrong";//存着wrong的字符串，5+1个单元 
	uint xdata solve_equation_count=0;
	if(x0==0.0 )
	{
		if(F1(x0,a,b,c,d,e)==0.0)
		{
			sprintf(x,"%g",(double)x0); 
			return 1;
		}
		else
			x0=x0+0.000001;
    ...
    switch( state )
    {
        ...
        case 2://计算
            ...
            else if( n==1 )
            {
                ...
                sprintf(temp_buff,"%g",(double)x_shuzhi); 
...
//                    temp_buff[i]='\0';//记得给字符串x加上结尾的 0 
                ...
                else
                {
                    ...
                    sprintf(temp_buff,"x2 =%g",(double)x_shuzhi);
...
//                    lcd_write_str(temp_buff); 
                    ...
                    sprintf(temp_buff,"x1 =%g",(double)x_shuzhi); 
