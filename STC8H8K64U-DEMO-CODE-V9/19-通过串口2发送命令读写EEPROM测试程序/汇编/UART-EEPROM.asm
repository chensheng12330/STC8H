...
L_MainLoop:
    ...
    CLR     C
    SUBB    A, #10
    JC      L_Jump_ProcessErr   ; RX2_Cnt < 10 bytes, jump
...
L_CheckLargeThan_z:
    INC     R0
    CJNE    R0, #(RX2_Buffer+8), L_CheckCharLoop
...
L_CalculateAddr:    
    ...
    MOV     R0, #RX2_Buffer+8
    CJNE    @R0, #' ', L_Jump_ProcessErr    ;检查命令是否正确   RX2_Buffer[8] = ' '
...
TestString1:
    DB  "STC8H8K64U系列单片机EEPROM测试程序!",0DH,0AH,0
TestString1A:
    DB  "W 0x0040 1234567890  --> 对0x0040地址写入字符1234567890.",0DH,0AH,0
TestString1B:
    DB  "R 0x0040 10          --> 对0x0040地址读出10个字节内容.",0DH,0AH,0
TestString2:
    DB  "读出",0
TestString3:
...
F_EEPROM_read_n:
    ...
    MOV     IAP_ADDRL, address_L    ; 送地址低字节
