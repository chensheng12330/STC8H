...
void main(void)
{
	...
	printf("STC8H UART2-DMA-SPI互为主从透传程序.\r\n");
...
void SPI_DMA_Config(void)
{
	...
	DMA_SPI_AMT = BUF_LENGTH;		//设置传输总字节数：n+1
...
void UART_DMA_Config(void)
{
	...
	DMA_UR2T_STA = 0x00;
	DMA_UR2T_AMT = BUF_LENGTH;	//设置传输总字节数：n+1
	...
	DMA_UR2R_STA = 0x00;
	DMA_UR2R_AMT = BUF_LENGTH;	//设置传输总字节数：n+1
...
void M2M_DMA_Config(void)
{
	...
	DMA_M2M_STA = 0x00;
	DMA_M2M_AMT = BUF_LENGTH;		//设置传输总字节数：n+1
