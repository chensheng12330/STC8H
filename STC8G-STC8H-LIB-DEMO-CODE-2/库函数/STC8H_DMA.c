...
void DMA_M2M_Inilize(DMA_M2M_InitTypeDef *DMA)
{
	...
	DMA_M2M_AMT = (u8)DMA->DMA_Length;				//设置传输总字节数：n+1
...
void DMA_SPI_Inilize(DMA_SPI_InitTypeDef *DMA)
{
	...
	DMA_SPI_AMT = (u8)DMA->DMA_Length;			//设置传输总字节数：n+1
...
void DMA_UART_Inilize(u8 UARTx, DMA_UART_InitTypeDef *DMA)
{
	...
	if(UARTx == UART1)
	{
		...
		DMA_UR1R_AMT = (u8)DMA->DMA_RX_Length;		//设置传输总字节数：n+1
	...
	if(UARTx == UART2)
	{
		...
		DMA_UR2R_AMT = (u8)DMA->DMA_RX_Length;		//设置传输总字节数：n+1
	...
	if(UARTx == UART3)
	{
		...
		DMA_UR3R_AMT = (u8)DMA->DMA_RX_Length;		//设置传输总字节数：n+1
	...
	if(UARTx == UART4)
	{
		...
		DMA_UR4R_AMT = (u8)DMA->DMA_RX_Length;		//设置传输总字节数：n+1
...
void DMA_LCM_Inilize(DMA_LCM_InitTypeDef *DMA)
{
	...
	DMA_LCM_AMT = (u8)DMA->DMA_Length;		//设置传输总字节数：n+1
